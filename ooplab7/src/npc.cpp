...
const int PEGASUS_MURDER_DISTANCE{50};
const int KNIGHT_MURDER_DISTANCE{10};
const int EICHHORN_MURDER_DISTANCE{10};

const int PEGASUS_MOVE_DISTANCE{10};
const int KNIGHT_MOVE_DISTANCE{30};
const int EICHHORN_MOVE_DISTANCE{10};
...
bool NPC::is_close(const std::shared_ptr<NPC> &other, size_t distance) {
    ...
    switch (this->type) {
    case PegasusType:
        distance = PEGASUS_MURDER_DISTANCE;
        break;
    case KnightType:
        distance = KNIGHT_MURDER_DISTANCE;
        break;
    case EichhornType:
        distance = EICHHORN_MURDER_DISTANCE;
        break;
    default:
        break;
    }
...
void NPC::move(int shift_x, int shift_y, int max_x, int max_y) {
    ...
    case PegasusType:
        if(shift_y % 2 == 0) {
            shift_x = PEGASUS_MOVE_DISTANCE*pow(-1, shift_x);
            shift_y = PEGASUS_MOVE_DISTANCE*pow(-1, shift_y);
        } else {
            shift_x = PEGASUS_MOVE_DISTANCE*pow(-1, shift_x);
            shift_y = PEGASUS_MOVE_DISTANCE*pow(-1, shift_y);
        }
    ...
    case EichhornType:
        if(shift_y % 2 == 0) {
            shift_x = EICHHORN_MOVE_DISTANCE*pow(-1, shift_x);
            shift_y = EICHHORN_MOVE_DISTANCE*pow(-1, shift_y);
        } else {
            shift_x = EICHHORN_MOVE_DISTANCE*pow(-1, shift_x);
            shift_y = EICHHORN_MOVE_DISTANCE*pow(-1, shift_y);
        }
