...
namespace My_Allocator {
    ...
    public:
        ...
        template <class U>
        struct rebind {
            using other = Allocator<U>; //возможность замены типа объектов, с которыми работает аллокатор, но сохраняя его другие свойства и методы
        };
