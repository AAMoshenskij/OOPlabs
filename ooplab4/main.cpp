...
int main() {    
    ...
    try {
        ...
        shared_ptr<Figure<double> > trapez_ptr(make_shared<trapez<double>>(10, 0, 1, 4, 1, 0));
        (*trapez_ptr) << cout;
        
        cout << trapez_ptr->figure_square_calc() << endl;
        vertex_t geo = trapez_ptr->geo_center_calc();
        cout << geo.first << " " << geo.second << endl;

        Array<shared_ptr<Figure<double> > > array{
                                    make_shared<Trapez<double>>(1, 2, 3, 0, 10, 5),
                                    make_shared<Square<double>>(1, 1, 4, 4, 1, 4, 4, 1),
                                    make_shared<Rectangle<double>>(10, 1, 15, 1, 15, 3, 10, 3),
                                    make_shared<Trapez<double>>(0, 0, 5, 5, 3, 0)
                                };
