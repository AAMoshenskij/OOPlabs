...
public:
    ...
    Trapez(T _x1, T _y1, T _x2, T _y2, T _x3, T _y3) {
        v1.first = _x1;
        v1.second = _y1;
        v2.first = _x2;
        v2.second = _y2;
    ...
    operator double() {                     //приводит к double, когда необходимо
        T sq = figure_square_calc();
        return static_cast<double>(sq);
    }

    Trapez operator=(Trapez<T>& t) {
    ...
    friend bool operator==(Trapez<T> t1, Trapez<T> t2) {
        int result = 1;
