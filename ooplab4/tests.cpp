...
TEST(TrapezMethods, square1) {
    Trapez<double> t(1, 2, 3, 3.6, 4.8, 2);
    double sq = t.figure_square_calc();
    ASSERT_TRUE(abs(sq - 3.04) < EPS);
}

TEST(TrapezMethods, square2) {
    Trapez<int> t(0, 0, 5, 5, 5, -5);
    double sq = t.figure_square_calc();
    ASSERT_TRUE(sq == 24);
}

TEST(TrapezMethods, square3) {
    Trapez<double> t;
    double sq = t.figure_square_calc();
    ASSERT_TRUE(abs(sq - 0) < EPS);
}

TEST(TrapezMethods, geo_center1) {
    Trapez<double> t(0, 0, 1, 1, 2, 0);
    using vertex_t = std::pair<double, double>;
    vertex_t v = t.geo_center_calc();
    ASSERT_TRUE((abs(v.first - 1) < EPS) && (abs(v.second - 1.0/3.0) < EPS));
}

TEST(TrapezMethods, geo_center2) {
    Trapez<int64_t> t;
    t.geo_center_calc();
...
TEST(TrapezOperatorsOverloads, square_op_overload) {
    Trapez<double> t(0, 0, 1, 1, 3, 1);
    double sq = t;
    ASSERT_TRUE(abs(sq - 1) < EPS);
}

TEST(TrapezOperatorsOverloads, equal_oper_overload1) {
    Trapez<int> t1(1, 1, 5, 1, 3, 7);
    Trapez<int> t2(1, 1, 5, 1, 3, 7);

    ASSERT_TRUE(t1 == t2);
}

TEST(TrapezOperatorsOverloads, equal_oper_overload2) {
    Trapez<long long> t1(1, 1, 5, 1, 3, 7);
    Trapez<long long> t2(0, 0, 5, 1, 3, 7);

    ASSERT_FALSE(t1 == t2);
}

TEST(TrapezOperatorsOverloads, assign_oper_overload1) {
    Trapez<int> t1(0, 0, 5, 1, 8, -5);
    Trapez<int> t2;
    t2 = t1;
...
TEST(TrapezOperatorsOverloads, assign_oper_overload3) {
    Trapez<bool> t1;
    Trapez<bool> t2;
    t1 = t2;
...
TEST(TrapezOperatorsOverloads, CopyConstructor1) {
    Trapez<int64_t> t1(0, 0, 5, 1, 8, -5);
    Trapez<int64_t> t2(t1);

    ASSERT_TRUE(t1 == t2);
}

TEST(TrapezOperatorsOverloads, cout_oper_overload1) {
...
TEST(TrapezOperatorsOverloads, cout_oper_overload2) {
    testing::internal::CaptureStdout(); 
    Trapez t1(1, 2, 3, 4, 6, -8);
    t1 << std::cout;
...
TEST(TrapezOperatorsOverloads, cout_oper_overload3) {
    testing::internal::CaptureStdout(); 
    Trapez t1(-7, 8, 3, 4, 6, -2);
    Trapez t2 = t1;
    t2 << std::cout;
    std::string output = testing::internal::GetCapturedStdout(); 
...
TEST(TrapezOperatorsOverloads, CinOperator1) {
    Trapez<double> t1;
    //1 1 5 0 4 10
    t1 >> std::cin;
    Trapez<double> t2(1, 1, 5, 0, 4, 10);
    ASSERT_TRUE(t1 == t2);
}

TEST(TrapezOperatorsOverloads, CinOperator2) {
    Trapez<double> t1;
    //1 1 5 0 4 11
    t1 >> std::cin;
    Trapez<double> t2(1, 1, 5, 0, 4, 10);
