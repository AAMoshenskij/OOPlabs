...
std::shared_ptr<NPC> factory(std::istream &is) {
    ...
    if (is >> type) {
        switch (type) {
        case EichhornType:
            result = std::make_shared<Eichhorn>(is);
            break;
        case KnightType:
...
std::shared_ptr<NPC> factory(NpcType type, int x, int y) {
    ...
    switch (type) {
    case EichhornType:
        std::cout << "Eichhorn generated\n";
        result = std::make_shared<Eichhorn>(x, y);
        break;
    case KnightType:
    ...
    for (const auto &attacker : array)
        for (const auto &defender : array)
            if ((attacker != defender) && (attacker->is_close(defender, distance))) {
                bool success{false};
                // if (defender->is_Eichhorn())
                //     success = attacker->fight(std::dynamic_pointer_cast<Eichhorn>(defender));
                // if (defender->is_knight())
                //     success = attacker->fight(std::dynamic_pointer_cast<Knight>(defender));
                // if (defender->is_Pegasus())
                //     success = attacker->fight(std::dynamic_pointer_cast<Pegasus>(defender));
                success = defender->accept(attacker);
                if (success)
